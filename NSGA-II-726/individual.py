...
class Individual():
    ...
    def __init__(self):
        self.env=Env()
        self.chromosome=[random.randint(0,self.env.n_actions-1) for i in range(10)]#三个一对
        ...
        self.fitness=[99,99,99,99,5000]#管道，罐底，换罐，用罐,能耗
    ...
    def decode(self):
        ...
        for a in self.chromosome:
            if not done:
                _,_,done=self.env.step(a)
        if self.env.SecureState and done:
            self.fitness=self.env.preTarget