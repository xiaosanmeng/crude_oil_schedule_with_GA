...
# def calRate(x):#------------------------新增-------------------------传入一个0~1的随机数，计算对应的泵速
#     for i in range(156):
...
def calRate(x):
    ...
    return 1375
def update_schduleOfpipe(tank,distiller,rate,schedules_pipe,environment:Environment,constant:Constant):
    ...
    if rate!=0:#非停运操作
        ...
        if(len(environment.emptyTK)!=0):
            ...
            endTime_a=urge_time-mintank_v/1250-constant.RESIDENCE_TIME
    ...
    for distiller in environment.undoneRT:#对每个为完成的蒸馏塔进行炼油
        ...
        if(distiller[2]!=0):#类型1还未完成
            for tank in environment.TK:#对每个罐进行搜索
                tankNum = tank[0]
                if(tank[1]==distiller[1] and tank[3]!=0 and environment.log_tank[tankNum-1][0]<=environment.time_ODF[distillerNum-1] and distiller[2]!=0):
                    #类型相同，存量不为0,并且可用(chargetime时间已过，充油完成
...
def decode(individual: Individual):
    chromosome=individual.chromosome#解
    ...
    # 解码开始前更新环境，更新emptyTK和undoneRT
    environment.update_undoneRT(environment.RT)
    environment.update_emptyTK(environment.TK)
    #--------------------解码开始--------------------------------------
    while(index<length and len(environment.undoneRT)>0 and environment.SecureState):

        if(len(environment.emptyTK)!=0):
            #根据染色体求出下标
            tankIndex=chromosome[index*3+0]%len(environment.emptyTK)
            distillerIndex=chromosome[index*3+1]%len(environment.undoneRT)
            rateIndex=chromosome[index*3+2]#0~1
            #根据下标求出需要的TK,DS编号
            ...
            #!!!!!根据randint（0，1)>>>>rate
            rate=calRate(rateIndex)


        else:
            tank=distiller=rate=0

        #更新ODF,炼油(预调度
        update_schduleOfdistiller(schedules_distiller,environment,constant)
        #更新ODT（与之前的方式不同，此时直接传入rate
        update_schduleOfpipe(tank,distiller,rate,schedules_pipe,environment,constant)
        # 更新ODF,炼油
        update_schduleOfdistiller(schedules_distiller, environment, constant)
        #更新emptyTK,undoneRT和SecureState
        environment.update_undoneRT(environment.RT)
        environment.update_emptyTK(environment.TK)
        environment.isSecureState()
        index+=1
    ...
    if(len(environment.undoneRT)==0):
        ...
        for i in range(len(individual.schedules_pipe)):
            rate=individual.schedules_pipe[i][5]#RATE取值=[0~1375]
            energy_time = (individual.schedules_pipe[i][4] - individual.schedules_pipe[i][3])#总时间
            rate_list=cal_rate(rate)#将泵速转为列表，1300=【0，0，0.6,0.4]

            energy_consumption=(rate_list[1]+2*rate_list[2]+3*rate_list[3])*energy_time
            individual.rate_list.append(rate_list)
            individual.fitness[4]+=energy_consumption
        #individual.fitness[4]/=10
